...
d_way_all_levels <- function(data_set) {
  data_set %>%
    group_by(K) %>%
    summarise(mean_lnprobdata = mean(abs(LnProbData)), sd_lnprobdata = sd(LnProbData)) %>%
    # calculate the variance between runs for each K
...
ggsave("plots/evanno_all_levels.pdf", (lnlikelihood_plot | a_way_plot | b_way_plot | c_way_plot | d_way_plot),
       height = 3, width = 15)
